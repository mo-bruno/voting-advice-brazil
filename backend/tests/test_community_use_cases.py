...
class FakePostRepository:
    ...
    def update_score(self, post_id: str, new_score: int) -> None:
        ...
        self._store[post_id] = Post(
            ...
            content=p.content,
            political_actor_id=p.political_actor_id,
...
def test_approved_post_is_saved():
    ...
    post, result = moderate_and_create_post(
        post_repo,
        log_repo,
        FakeModerationClient(approved=True),
        anonymous_id="anon-1",
        content="PT votou contra a PEC",
    )
...
def test_rejected_post_not_saved():
    ...
    post, result = moderate_and_create_post(
        post_repo,
        log_repo,
        FakeModerationClient(approved=False, reason="Fora do tema"),
        anonymous_id="anon-1",
...
def test_get_post_with_comments():
    ...
    comment_repo.create(
        Comment(
            id="c1", post_id="p1", anonymous_id="a", content="ótimo", created_at=_now()
        )
    )
...
def test_create_comment_returns_none_if_post_missing():
    ...
    assert (
        create_comment(
            FakePostRepository(), FakeCommentRepository(), "nope", "anon", "x"
        )
        is None
    )
...
def test_vote_returns_none_if_post_missing():
    ...
    assert (
        vote_post(FakePostRepository(), FakeVoteRepository(), "nope", "anon", 1) is None
    )
